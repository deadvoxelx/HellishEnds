...
NetherVineTile::NetherVineTile(int id) : Tile(id, Material::replaceable_plant, isSolidRender() )
{
	setTicking(true);
}

void NetherVineTile::updateDefaultShape()
{
	setShape(0, 0, 0, 1, 1, 1);
}

int NetherVineTile::getRenderShape()
{
	return SHAPE_VINE;
}

bool NetherVineTile::isSolidRender(bool isServerLevel)
{
	return false;
}

bool NetherVineTile::isCubeShaped()
{
	return false;
}

void NetherVineTile::updateShape(LevelSource *level, int x, int y, int z, int forceData, shared_ptr<TileEntity> forceEntity) // 4J added forceData, forceEntity param
{
...
AABB *NetherVineTile::getAABB(Level *level, int x, int y, int z)
{
	return NULL;
}

bool NetherVineTile::mayPlace(Level *level, int x, int y, int z, int face)
{
	switch (face)
	{
	default:
...
bool NetherVineTile::isAcceptableNeighbor(int id)
{
...
bool NetherVineTile::updateSurvival(Level *level, int x, int y, int z)
{
	int facings = level->getData(x, y, z);
...
int NetherVineTile::getColor() const
{
	return FoliageColor::getDefaultColor();
}

int NetherVineTile::getColor(int auxData)
{
	return FoliageColor::getDefaultColor();
}

int NetherVineTile::getColor(LevelSource *level, int x, int y, int z, int data)
{
	return getColor(level, x, y, z);
}

int NetherVineTile::getColor(LevelSource *level, int x, int y, int z)
{
	return level->getBiome(x, z)->getFolageColor();
}

void NetherVineTile::neighborChanged(Level *level, int x, int y, int z, int type)
{
...
void NetherVineTile::tick(Level *level, int x, int y, int z, Random *random)
{
...
int NetherVineTile::getPlacedOnFaceDataValue(Level *level, int x, int y, int z, int face, float clickX, float clickY, float clickZ, int itemValue)
{
	int facings = 0;
	switch (face)
	{
...
int NetherVineTile::getResource(int data, Random *random, int playerBonusLevel)
{
	return Tile::netherVine->id;
}

int NetherVineTile::getResourceCount(Random *random)
{
	return 1;
}

void NetherVineTile::playerDestroy(Level *level, shared_ptr<Player>player, int x, int y, int z, int data)
{
	if (!level->isClientSide && player->getSelectedItem() != NULL && player->getSelectedItem()->id == Item::shears->id)
	{
		...
		popResource(level, x, y, z, shared_ptr<ItemInstance>(new ItemInstance(Tile::netherVine, 1, 0)));
	}
	else
	{
