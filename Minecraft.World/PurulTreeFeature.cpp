...
PurulTreeFeature::PurulTreeFeature(int forcedType) : Feature(true)
{
	this->forcedType = forcedType;
}

PurulTreeFeature::PurulTreeFeature() : Feature(false)
{
	this->forcedType = -1;
}

bool PurulTreeFeature::place(Level *level, Random *random, int x, int y, int z)
{
	int type = 1;
		...
		for (int xx = x - r; xx <= x + r && free; xx++)
		{
			for (int zz = z - r; zz <= z + r && free; zz++)
			{
				if (yy >= 0 && yy < Level::maxBuildHeight)
				{
					...
					if (tt != 0 && tt != Tile::purulVeloett_Id)
					{
						free = false;
					}
				}
	...
	if (belowTile != Tile::veloettGrass_Id)
	{
		return false;
	}
	...
	for (int yy = low; yy <= y + treeHeight; yy++)
	{
		...
		for (int xx = x - offs; xx <= x + offs; xx++)
		{
			for (int zz = z - offs; zz <= z + offs; zz++)
			{
				...
				{
					if (!Tile::solid[level->getTile(xx, yy, zz)]) placeBlock(level, xx, yy, zz, Tile::purulVeloett_Id, 0);
				}
			}
		}
	}
	for (int hh = 0; hh < treeHeight; hh++)
	{
		...
		if (!Tile::solid[t]) placeBlock(level, x, y + hh, z, Tile::purulLog_Id, 0);
	}
	return true;
}
